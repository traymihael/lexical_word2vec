...
class ContinuousBoW(chainer.Chain):
    ...
    def __init__(self, n_vocab, n_units, loss_func, ori_con_data):
        ...
        with self.init_scope():
            ...
            # self.embed.W.data[7] = -0.00035557806
            # self.embed.W.data[14] = -0.00035557806
...
class WindowIterator(chainer.dataset.Iterator):
    ...
    def __next__(self):
                ...
                if data_mi[k][0] in contexts_kari[i]:
                    ...
                    for j in range(contexts_num):
                        if kari[j] != data_mi[k][0]:
                            kari[j] = last_num
                    ins_context.append([i+1, kari])
                    ins_center.append([i+1, center[i]])

        ins_context.sort(key=lambda x: x[0])
        ...
        if flg:
            ...
            for i in range(len(ins_context)):
                contexts.append(ins_context[i][1])
                center.append(ins_center[i][1])
                count += 1

        # 7 8 9 10 14
        center = [9, 9 ,10, 10]
        contexts = [[7,10], [14,16],[7,9],[14,16]]
        contexts = [[7, 10], [3, 16], [7, 9], [3, 16]]
...
def main():
    ...
    parser.add_argument('--negative-size', default=2, type=int,
